...
class tripleo::network::contrail::issu(
  ...
  $host_ip_control              = hiera('contrail_issu_host_ip_control', $host_ip),
  $ibgp_auto_mesh               = true,
  $issu_ips                     = hiera('contrail_issu_node_ips', undef),
  $issu_ips_compute             = hiera('contrail_issu_compute_node_ips', undef),
  ...
  $revert_issu_api_info = join(['{"', join([join($old_config_servers, '": [("root"), ("")],"', ''), '": [("root"), ("")]}'], '')], '')
  ...
  if $issu_ips {
    $issu_control_servers_list_comma = join($issu_ips_compute, ',')
  } else {
