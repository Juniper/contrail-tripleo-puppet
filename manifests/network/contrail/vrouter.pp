...
class tripleo::network::contrail::vrouter (
  ...
  if size(hiera('contrail::vrouter::control_node_ips')) == 0 {
    ...
    $control_server_list = join(hiera('contrail::vrouter::control_node_ips'), ' ')
    $control_server_list_53 = join([join(hiera('contrail::vrouter::control_node_ips'), ':53 '),':53'],'')
    $control_server_list_5269 = join([join(hiera('contrail::vrouter::control_node_ips'), ':5269 '),':5269'],'')
  }
  if $auth_version == 2 {
    $keystone_config_ver = {}
