...
for ip in $issu_ips_list_space ; do
  ...
  proviosion_control $ip $name $old_api_server_ip $old_api_server_port|| {
...
if [[ "$oper" == 'del' && "$step" == 'pair_with_old' ]] ; then
  ...
  for ip in $old_control_servers_list_space ; do
    ...
    proviosion $ip $name $issu_api_server_ip $issu_api_server_port provision_config_node.py $config_container_id
  done
  for ip in $old_analytics_servers_list_space ; do
    name=$(resolve_name $ip pair_with_old)
    proviosion $ip $name $issu_api_server_ip $issu_api_server_port provision_analytics_node.py $analytics_container_id
  done
  for ip in $old_analyticsdb_servers_list_space ; do
    name=$(resolve_name $ip pair_with_old)
    proviosion $ip $name $issu_api_server_ip $issu_api_server_port provision_database_node.py $analyticsdb_container_id
  done
fi
